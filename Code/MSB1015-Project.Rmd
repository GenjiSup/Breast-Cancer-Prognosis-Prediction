...
required_packages <- c("rstudioapi", "readr", "dplyr", "ggplot2", "tidyr", 
...
# Create a function to form the prognosis variable
...
categorical_data <- data_filtered %>%
  ...
  mutate(
    prognosis = as.factor(categorise_prognosis(overall_survival_months)) # Call the function to create 'prognosis'
  ) %>%
...
get_final_metrics <- function(results_data) {
  ...
  t_final_metrics_df <- t_final_metrics_df %>%
    rowwise() %>%
    mutate(average = mean(c_across(everything()), na.rm = TRUE)) %>%
    ungroup() %>%
    mutate(Metric = c("AUC", "Balanced Accuracy"))
...
pred_table_training <- table(best_class_data$best_model_overall$output$predy, best_class_data$best_model_overall$output$testy)
train_confusion <- confusionMatrix(pred_table_training)
train_confusion

# Call the plotting function for the confusion matrix
source("~/University/MSB/MSB1015/confusionPlot.R") # Change the directory as needed
...
pred_probs <- predict(best_class_data$best_model_overall, newdata = best_class_data$train, type = "prob")
positive_class_probs <- pred_probs[, "will_die_within_10_years"]

# Get the precision recall values
best_class_data$best_model_overall$prc <- prc(best_class_data$best_model_overall)
...
plot(best_class_data$best_model_overall$roc, col = "red", main = "ROC Training", las = 1)
legend("bottomright", legend = paste0("AUC = ", signif(pROC::auc(best_class_data$best_model_overall$roc), 3)), bty = "n")
plot(best_class_data$best_model_overall$prc, col = "red", main = "Precision-recall Training")
legend("topright", legend = paste0("AUC = ", round(best_class_data$best_model_overall$prc$auc, 3)), bty = "n")
...
importance <- as.data.frame(best_class_data$best_model_overall$final_fit$finalModel$importance) %>%
  mutate(
    will_die_within_10_years = will_die_within_10_years * 10,
    will_die_within_5_years = will_die_within_5_years * 10,
    column = row.names(best_class_data$best_model_overall$final_fit$finalModel$importance)
  )

importance_sorted <- importance[order(importance$will_die_within_10_years, decreasing = TRUE), ]
...
err_rate <- as.data.frame(best_class_data$best_model_overall$final_fit$finalModel$err.rate) %>%
  mutate(ntrees = 1:nrow(best_class_data$best_model_overall$final_fit$finalModel$err.rate))
...
pr_curve <- pr.curve(
  scores.class0 = positive_class_probs,
  weights.class0 = as.numeric(best_class_data$test$prognosis == "will_die_within_10_years"),
  curve = TRUE
)
...
op <- par(mfrow = c(1, 2), mar = c(4, 4, 2, 2) + 0.1)
plot(best_class_data$best_model_overall$roc, col = "red", main = "ROC", las = 1)
...
plot(best_class_data$best_model_overall$prc, col = "red", main = "Precision-Recall Curve", xlab = "Recall", ylab = "Precision")
...
pred_table <- table(pred_best_threshold, best_class_data$test$prognosis)
dimnames(pred_table) <- list(
