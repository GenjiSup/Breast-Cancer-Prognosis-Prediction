...
# Load the function for PCoA
source("~/University/MSB/MSB1015/funPCoA.R") # Change the directory as needed

# Load the confusion function
...
PCoA_plots_rawdata <- PCoA_proximity_plot(data, "tumor_stage")
grid.arrange(grobs = PCoA_plots_rawdata, ncol = 2)
...
# Call the function for the unsupervised random forest PCoA 
PCoA_plots_rawdata <- PCoA_proximity_plot(data, "tumor_stage", n_trees = 10)
grid.arrange(grobs = PCoA_plots_rawdata, ncol = 2)
...
# Call the function for the unsupervised random forest PCoA
PCoA_plots_categorical <- PCoA_proximity_plot(categorical_data, "prognosis")
grid.arrange(grobs = PCoA_plots_categorical, ncol = 2)
...
# Call the function for the unsupervised random forest PCoA (will take a few minutes)
PCoA_plots_genetic <- PCoA_proximity_plot(genetic_data, "prognosis")
grid.arrange(grobs = PCoA_plots_genetic, ncol = 2)
...
# Call the function for the unsupervised random forest PCoA (will take a few minutes)
PCoA_plots_complete <- PCoA_proximity_plot(complete_data, "prognosis")
grid.arrange(grobs = PCoA_plots_complete, ncol = 2)
```
...
random_forest <- function(data, ntree, nrun) {
  ...
  # Initialize lists to store all train and test splits
  train_splits <- vector("list", nrun)
  test_splits <- vector("list", nrun)
  ...
  for (repeat_run in 1:nrun) {
    ...
    # Store the train and test sets for this run
    train_splits[[repeat_run]] <- train
    test_splits[[repeat_run]] <- test
  ...
  return(list(best_model_overall = best_model_overall, 
              store_models_repeated = store_models_repeated, 
...
# Categorical results
results_categorical <- random_forest(categorical_data, ntree = 1000, nrun = 5)

# Genetic results
results_genetic <- random_forest(genetic_data, ntree = 10, nrun = 1)

# Complete results
results_complete <- random_forest(complete_data, ntree = 10, nrun = 1)
```
...
# 3.3 Finding the matching splits for the best model
```{r finding matching splits}
# Get the AUC value of the best model
best_auc <- best_class_data$best_model_overall$roc$auc

# Initialize variables to store the matching run and model
matching_run <- NULL
matching_model <- NULL
matching_index <- NULL

# Loop over all runs in store_models_repeated
for (run_index in seq_along(best_class_data$store_models_repeated)) {
  # Get the current run's list of models
  current_run_models <- best_class_data$store_models_repeated[[run_index]]
  
  # Loop over all models in the current run and compare their AUC values
  for (model_index in seq_along(current_run_models)) {
    model_name <- names(current_run_models)[model_index]
    model_auc <- current_run_models[[model_name]]$roc$auc
    
    # Check if the AUC matches the best model's AUC
    if (model_auc == best_auc) {
      matching_run <- run_index
      matching_model <- model_name
      matching_index <- model_index
      break
    }
  }
  
  # Break out of the outer loop if a match is found
  if (!is.null(matching_run)) {
    break
  }
}

# If a match is found, print the run, model name, and retrieve the corresponding train/test splits
if (!is.null(matching_run) && !is.null(matching_model)) {
  print(paste("Matching model found in run:", matching_run, "model:", matching_model))

  # Retrieve the corresponding train and test splits
  matching_train_split <- best_class_data$train_splits[[matching_run]]
  matching_test_split <- best_class_data$test_splits[[matching_run]]
  
  # Print or use the splits as needed
  print("Found Matching Splits")
...
pred_probs <- predict(best_class_data$best_model_overall, newdata = matching_train_split, type = "prob") # Predict probabilities
...
legend("bottomright", legend = paste0("AUC = ", signif(pROC::auc(best_class_data$best_model_overall$roc), 2)), bty = "n") # Add AUC legend
plot(best_class_data$best_model_overall$prc, col = "red", main = "Precision-recall Training") # Plot Precision-Recall curve
legend("topright", legend = paste0("AUC = ", round(best_class_data$best_model_overall$prc$auc, 2)), bty = "n") # Add AUC legend for PRC
...
# Get predicted probabilities for the positive class ("will_die_within_10_years")
pred_probs <- predict(best_class_data$best_model_overall, newdata = matching_test_split, type = "prob")
...
roc_curve <- roc(matching_test_split$prognosis, positive_class_probs, levels = levels(matching_test_split$prognosis), direction = ">")

# Create Precision-Recall curve for the test set
pr_curve <- pr.curve(
  scores.class0 = positive_class_probs, # Probabilities for the positive class
  weights.class0 = as.numeric(matching_test_split$prognosis == "will_die_within_10_years"), # Binary vector for positive class
...
pred_best_threshold <- factor(pred_best_threshold, levels = levels(matching_test_split$prognosis))

# Calculate accuracy on the test set using the best threshold
pred_table <- table(pred_best_threshold, matching_test_split$prognosis) # Create confusion matrix table
