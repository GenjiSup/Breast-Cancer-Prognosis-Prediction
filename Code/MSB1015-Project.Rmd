...
lower_threshold <- mean_score - 2*sd_score

# Filter out rows where the score is outside of 2 standard deviations from the mean
...
# Create a function to create the prognosis variable
...
# 2.1 The random forest
```{r random forest, echo=FALSE}

# Set the seed for reproducibility of random results using the L'Ecuyer-CMRG method
...
trainIndex <- createDataPartition(categorical_data$prognosis, p = .7, 
                                  ...
                                  times = 1)     # Perform the partitioning once

# Subset the original data to create the training set using the indices from trainIndex
train <- categorical_data[trainIndex,]

# Subset the original data to create the testing set using the remaining indices
...
# When creating the model these values can be changed.
ntrees <- c(2000) 
nodesize <- c(1)

# List for the combinations: should have all the possible combinations between ntrees and nodesize.
list <- 1
...
# 2.3 ROC and PR curves on the best model
